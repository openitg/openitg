...
public:
	...
	{
	public:
		ValueTypeMismatch(const std::string &name, const type_info &stored, const type_info &retrieving)
			: InvalidArgument("NameValuePairs: type mismatch for '" + name + "', stored '" + stored.name() + "', trying to retrieve '" + retrieving.name() + "'")
			, m_stored(stored), m_retrieving(retrieving) {}

		const type_info & GetStoredTypeInfo() const {return m_stored;}
		const type_info & GetRetrievingTypeInfo() const {return m_retrieving;}

	private:
		const type_info &m_stored;
		const type_info &m_retrieving;
	};
	...
	CRYPTOPP_DLL static void CRYPTOPP_API ThrowIfTypeMismatch(const char *name, const type_info &stored, const type_info &retrieving)
	...
	CRYPTOPP_DLL virtual bool GetVoidValue(const char *name, const type_info &valueType, void *pValue) const =0;
};
