...
//Added for DDR Satellites -- i think windows only...
void
Serial::SerialImpl::setMask (uint16_t mask)
{
  if (is_open_ == false) {
    throw PortNotOpenedException ("Serial::setMask");
  }
	if (!SetCommMask(fd_, mask)) {
	// Error setting communications mask
	THROW (IOException, "Error setting the comm event mask!");
	}
}
