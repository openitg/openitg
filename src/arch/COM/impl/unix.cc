...
Serial::SerialImpl::reconfigurePort ()
{
  ...
  //hack this so RTS is on but CTS is off.... i dont have a unix system to test this with -- same as FC_hardware here
...
//added for ddr satellites
void
Serial::SerialImpl::setMask (uint16_t mask)
{
	stringstream ss;
    ss << "setMask not supported on *nix! Maybe its not needed?";
    //throw(SerialException(ss.str().c_str()));
}

//added for ddr satellites -- this is what WINE does...
bool
Serial::SerialImpl::purgeComm (uint16_t dwFlags)
{
	 if (dwFlags & 0x01) flushOutput ();
